...
public class Zombie : LivingEntity
{
    ...
    public void Setup(ZombieData zombieData) {
        ...
        // 렌더러가 사용 중인 머티리얼의 컬러를 번경, 외형 색이 변함
    ...
    private IEnumerator UpdatePath() {
        ...
        while (!dead)
        {
            ...
            else
            {
                ...
                // 20유닛의 반지름의 가진 가상의구름 그렸을 때 구와 겹치는 모든 콜라이더를 가져옴
                ...
                for (int i = 0; i < colliders.Length; i++)
                {
                    ...
                    // LivingEntity 컴포넌트가 존재하며, 해당 LivingEntity 가 살아 있다면
                    if (livingEntity != null && !livingEntity.dead)
                    {
                        //추적 대상을 해당 LivingEntity 로 설정
    ...
    private void OnTriggerStay(Collider other) {
        // 자신이 사망하지 않았으며,
        // 최신 공격 시점에서 timeBetAttack 이상 사건이 지났다면 공격 가능
